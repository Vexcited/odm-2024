...
const ReservationCard: Component<IReservation> = (reservation) => {
  return (
    <div class="bg-white relative flex h-250px w-fit border-2 rounded-3xl">
      ...
      <img class="w-300px h-full object-cover rounded-l-xl" src={`/cdn/${reservation.trip.previewImage}`} />
      <div class="w-320px flex flex-col justify-between p-4">
        <div class="">
          <div class="flex justify-between gap-4">
            ...
            <p class="flex-shrink-0">
              <strong class="font-600">{reservation.trip.pricePerNight} €</strong> / nuit
            </p>
          </div>

          <p class="text-xs">
            {reservation.trip.city}, {reservation.trip.country}
          </p>
          ...
          <div class="flex flex-col mt-4 gap-2 text-black/90">
            <div class="flex items-center gap-2">
              ...
              <p class="">
                du <span class="font-500">{new Date(reservation.from).toLocaleDateString()}</span>
              </p>
            </div>
            <div class="flex items-center gap-2">
              <MdiClockEnd class="" />
              <p class="text-sm">
                au <span class="font-500">{new Date(reservation.to).toLocaleDateString()}</span>
              </p>
            </div>
            <div class="flex items-center gap-2">
              <MdiAccount class="" />
              <p class="text-sm">
                {reservation.amountOfPeople} personne(s)
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="flex gap-2">
          ...
          <a
            ...
            href={`/trip/${reservation.trip._id}`}
